...
export function AsianRecipes() {
  const [selectedCuisine, setSelectedCuisine] = useState<string>("")
  const [recipes, setRecipes] = useState<AsianRecipe[]>([])
  const [isLoading, setIsLoading] = useState(false)
  const [selectedRecipe, setSelectedRecipe] = useState<AsianRecipe | null>(null)
  ...
  const handleCuisineSelect = async (cuisine: string) => {
    ...
    try {
      const response = await fetch(`/api/asian-recipes?cuisine=${cuisine}`)
      ...
      console.error("Failed to fetch Asian recipes:", error)
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <Card className="glass-effect border-blue-200/40 shadow-xl">
        <CardHeader className="text-center">
          ...
          <CardDescription className="text-base">
            Explore authentic recipes from across Asia - all free from TheMealDB
          </CardDescription>
        </CardHeader>
      ...
      {selectedRecipe && (
        <RecipeModal
          ...
          recipe={{
            ...
            time: "30-45 min",
            difficulty: "Medium",
            tags: [selectedRecipe.cuisine.toLowerCase()],
            variations: {
              standard: "Traditional preparation with authentic ingredients",
              dairyFree: "Naturally dairy-free in most Asian cuisines",
              vegan: "Use tofu or plant-based proteins instead of meat",
            },
            servings: 2,
            healthScore: 80,
